...
class ResNet(nn.Module):
    ...
    def __init__(self,
        ...
        self.conv1 = nn.Conv2d(3, self.in_channel, kernel_size=7, stride=2,
                               padding=3, bias=False) # 7x7,64,stride2,padding3
        self.bn1 = nn.BatchNorm2d(self.in_channel)
        self.relu = nn.ReLU(inplace=True)
        self.maxpool = nn.MaxPool2d(kernel_size=3, stride=2, padding=1) # 3x3,stride2,padding1
    ...
    def _make_layer(self, block, channel, block_num, stride=1):
        ...
        if stride != 1 or self.in_channel != channel * block.expansion:
            # 需要进行downsample以控制形状相同才能add
            # stride != 1 让图像尺寸不同
            # in_channel != channel * block.expansion 让channel不同
            downsample = nn.Sequential(
                nn.Conv2d(self.in_channel, channel * block.expansion, kernel_size=1, stride=stride, bias=False),
                nn.BatchNorm2d(channel * block.expansion))

        layers = []
        # 每个layer的第一个Bottleneck会有downsample,剩下的Bottleneck没有
        ...
        # 每个layer的第二个Bottleneck开始，通过Bottleneck的in_channel 和 out_channel都一样了，因此不再需要expansion
